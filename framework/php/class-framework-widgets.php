...
class ace_tweetbox extends WP_Widget {
	
	function ace_tweetbox() {
		//Constructor
	...
	function form($instance) {
		...
		$instance = wp_parse_args( (array) $instance, array( 'title' => 'Latest Tweets', 'count' => '3', 'username' => ace_get_option('twitter') ) );
...
class ace_newsbox extends WP_Widget {

	var $ace_term = '';
	var $ace_post_type = '';
	var $ace_new_query = '';

	function ace_newsbox() 
	{
	...
	{	
		...
		else
		{
			...
			if(empty($catarray[0]))
			{
				$new_query = array("posts_per_page"=>$count,"post_type"=>$this->ace_post_type);
			}
			else
			{
				if($this->ace_new_query)
				{
					$new_query = $this->ace_new_query;
				}
				else
				{
					$new_query = array(	"posts_per_page"=>$count, 'tax_query' => array( 
													array( 'taxonomy' => $this->ace_term, 
														   'field' => 'id', 
														   'terms' => explode(',', $cat), 
														   'operator' => 'IN')
														  )
		...
		$image = "";
		$slides = ace_post_meta(get_the_ID(), 'slideshow', true);
		
		if( $slides != "" && !empty( $slides[0]['slideshow_image'] ) )
		{
			$image = ace_image_by_id($slides[0]['slideshow_image'], 'widget', 'image');
		}
	...
	function form($instance) 
	{
		...
		if(!empty($this->ace_term))
		{
			$elementCat['taxonomy'] = $this->ace_term;
		}
		
		
		
		
		$html = new ace_htmlhelper();
...
class ace_portfoliobox extends ace_newsbox
{
	function ace_portfoliobox() 
	{
		
		$this->ace_term = 'portfolio_entries';
		$this->ace_post_type = 'portfolio';
		$this->ace_new_query = ''; //set a custom query here
...
class ace_socialcount extends WP_Widget {
	
	function ace_socialcount() {
		//Constructor
		$widget_ops = array('classname' => 'ace_socialcount', 'description' => 'A widget to display your twitter and rss followers' );
		$this->WP_Widget( 'ace_socialcount', THEMENAME.' Twitter and RSS count', $widget_ops );
	}
	...
	function widget($args, $instance) {
		...
		if(!empty($follower) && is_array($follower))
		{
			...
			if(isset($follower['twitter']))
			{
				...
				echo "<a href='$link' class='asc_twitter $addClass'><strong class='asc_count'>".$follower['twitter']."</strong><span>".__('Follower','ace_framework')."</span></a>";
			}
			
			if(isset($follower['rss']))
			{
				$link = 'http://feeds.feedburner.com/'.$rss;
				echo "<a href='$link' class='asc_rss $addClass'><strong class='asc_count'>".$follower['rss']."</strong><span>".__('Subscribers','ace_framework')."</span></a>";
			}
	...
	function form($instance) {
		...
		$instance = wp_parse_args( (array) $instance, array('rss' => ace_get_option('feedburner'), 'twitter' => ace_get_option('twitter') ) );
...
class ace_partner_widget extends WP_Widget {
	
	function ace_partner_widget() {
		
		$this->add_cont = 2;
		//Constructor
		$widget_ops = array('classname' => 'ace_partner_widget', 'description' => 'An advertising widget that displays 2 images with 125 x 125 px in size' );
		$this->WP_Widget( 'ace_partner_widget', THEMENAME.' Advertising Area', $widget_ops );
	}
	...
	function widget($args, $instance) 
	{
		...
		global $aceaddwidget, $firsttitle;
		$aceaddwidget ++;
		
		$title = empty($instance['title']) ? '' : apply_filters('widget_title', $instance['title']);
		$image_url = empty($instance['image_url']) ? '<span class="ace_parnter_empty"><span>'.__('Advertise here','ace_framework').'</span></span>' : '<img class="rounded" src="'.$instance['image_url'].'" title="" alt=""/>';
		$ref_url = empty($instance['ref_url']) ? '#' : apply_filters('widget_comments_title', $instance['ref_url']);
		$image_url2 = empty($instance['image_url2']) ? '<span class="ace_parnter_empty"><span>'.__('Advertise here','ace_framework').'</span></span>' : '<img class="rounded" src="'.$instance['image_url2'].'" title="" alt=""/>';
		$ref_url2 = empty($instance['ref_url2']) ? '#' : apply_filters('widget_comments_title', $instance['ref_url2']);

		if ( !empty( $title ) ) { echo $before_title . $title . $after_title; };
		echo '<a href="'.$ref_url.'" class="preloading_background  ace_partner1 link_list_item'.$aceaddwidget.' '.$firsttitle.'" >'.$image_url.'</a>';
		if($this->add_cont == 2) echo '<a href="'.$ref_url2.'" class="preloading_background ace_partner2 link_list_item'.$aceaddwidget.' '.$firsttitle.'" >'.$image_url2.'</a>';
		echo $after_widget;
...
//one image
class ace_one_partner_widget extends ace_partner_widget
{
	function ace_one_partner_widget() 
	{
		
		$this->add_cont = 1;
		
		$widget_ops = array('classname' => 'ace_one_partner_widget', 'description' => 'An advertising widget that displays 1 big image' );
		
		$this->WP_Widget( 'ace_one_partner_widget', THEMENAME.' Big Advertising Area', $widget_ops );
	}
}
...
class ace_combo_widget extends WP_Widget {
	
	function ace_combo_widget() {
		//Constructor
		$widget_ops = array('classname' => 'ace_combo_widget', 'description' => 'A widget that displays your popular posts, recent posts, recent comments and a tagcloud' );
		$this->WP_Widget( 'ace_combo_widget', THEMENAME.' Combo Widget', $widget_ops );
	}
	...
	function widget($args, $instance) 
	{
		...
		echo '<div class="tab first_tab widget_tab_popular"><span>'.__('Popular', 'ace_framework').'</span></div>';
		echo "<div class='tab_content'>";
		ace_get_post_list('cat=&orderby=comment_count&posts_per_page='.$posts);
		echo "</div>";
		
		echo '<div class="tab widget_tab_recent"><span>'.__('Recent', 'ace_framework').'</span></div>';
		echo "<div class='tab_content'>";
		ace_get_post_list('showposts='. $posts .'&orderby=post_date&order=desc');
		echo "</div>";
		
		echo '<div class="tab widget_tab_comments"><span>'.__('Comments', 'ace_framework').'</span></div>';
		echo "<div class='tab_content'>";
		ace_get_comment_list( array('number' => $posts, 'status' => 'approve', 'order' => 'DESC') );
		echo "</div>";
		
		echo '<div class="tab last_tab widget_tab_tags"><span>'.__('Tags', 'ace_framework').'</span></div>';
...
get posts posts
-----------------------------------------------------------------------------------*/
if (!function_exists('ace_get_post_list')) 
{
	function ace_get_post_list( $ace_new_query , $excerpt = false) 
	{
		global $ace_config;
		
		$additional_loop = new WP_Query($ace_new_query);
		...
		//check for preview images:
		$image = "";
		$slides = ace_post_meta(get_the_ID(), 'slideshow');
		...
		echo $image;
		echo "</span>";
		echo "<strong class='news-headline'>".ace_backend_truncate(get_the_title(), 55," ");
		echo "<span class='news-time'>".get_the_time("F j, Y, g:i a")."</span>";
		echo "</strong>";
		echo "</a>";
...
if (!function_exists('ace_get_comment_list')) 
{
	function ace_get_comment_list($ace_new_query) 
	{
		global $ace_config;
		
		$comments = get_comments($ace_new_query);
		...
		echo "</span>";
		echo "<strong class='news-headline'>".ace_backend_truncate($comment->comment_content, 55," ");
