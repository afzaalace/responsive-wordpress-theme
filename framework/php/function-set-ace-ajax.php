...
if(!function_exists('ace_ajax_modify_set'))
{
	function ace_ajax_modify_set()
	{	
		...
		if($_POST['method'] == 'add')
		{
			...
			if(isset($_POST['context']))
			{
				...
				{
					include( ACE_BASE.'/includes/admin/register-admin-metabox.php' );
				...
				{
					if(! @include(ACE_BASE.$_POST['configFile'])) include($_POST['configFile']);
					$sets->elements = $elements;
				}
			}
			...
			if($element)
			{
				...
				{
					add_filter('ace_ajax_render_element_filter', $_POST['activate_filter'], 10, 2);
				}
				
				$element = apply_filters('ace_ajax_render_element_filter', $element, $_POST);
				
				//render element for output
				echo "{ace_ajax_element}" .$html->render_single_element($element) ."{/ace_ajax_element}";
	...
	add_action('wp_ajax_ace_ajax_modify_set', 'ace_ajax_modify_set');
}

//helper function for the gallery that fetches all image atachment ids of a post
function ace_ajax_fetch_all($element, $sent_data)
{
...
/**
 * This function receives the values entered into the option page form elements. All values are submitted via ajax (js/ace_framwork.js)
 ...
 */
if(!function_exists('ace_ajax_save_options_page'))
{
	function ace_ajax_save_options_page()
	{
		
		//check if user is allowed to save and if its his intention with a nonce check
		if(function_exists('check_ajax_referer')) { check_ajax_referer('ace_nonce_save_backend'); }
		...
		$store_me = ace_ajax_save_options_create_array($data_sets);
		...
		if(isset($_POST['dynamicOrder']) && $_POST['dynamicOrder'] != "")
		{
			...
			{
				if(in_array($element['slug'], $ace->subpages[$_POST['slug']]))
				{
		...
		update_option('ace_rewrite_flush', 1);
		
		die('ace_save');
	}
	
	//hook into wordpress admin.php
	add_action('wp_ajax_ace_ajax_save_options_page', 'ace_ajax_save_options_page');
}



/**
 *  ace_ajax_save_options_create_array
 *  
 ...
 */
if(!function_exists('ace_ajax_save_options_create_array'))
{
	function ace_ajax_save_options_create_array($data_sets, $global_post_array = false)
	{
		$result = array();
		$charset = get_bloginfo('charset');
 ...
 */
if(!function_exists('ace_ajax_reset_options_page'))
{
	function ace_ajax_reset_options_page()
	{
		//check if user is allowed to reset and if its his intention with a nonce check
		if(function_exists('check_ajax_referer')) { check_ajax_referer('ace_nonce_reset_backend'); }
		
		global $ace, $wpdb;
		$slugs = array($ace->option_prefix, $ace->option_prefix.'_dynamic_elements', $ace->option_prefix.'_dynamic_pages');
		
		//get all option keys of the framework
		/*
		foreach($ace->option_pages as $option_page)
				{
					if($option_page['slug'] == $option_page['parent'])
					{
						$slugs[$ace->option_prefix.'_'.$option_page['slug']] = true;
					}
				}
		*/
		...
		//end php execution and return ace_reset to the javascript
		die('ace_reset');
	}
	
	//hook into wordpress admin.php
	add_action('wp_ajax_ace_ajax_reset_options_page', 'ace_ajax_reset_options_page');
}
 ...
 */
if(!function_exists('ace_ajax_get_image'))
{
	function ace_ajax_get_image()
	{
	...
	//hook into wordpress admin.php
	add_action('wp_ajax_ace_ajax_get_image', 'ace_ajax_get_image');
}



if(!function_exists('ace_ajax_get_gallery'))
{
	function ace_ajax_get_gallery()
	{
		#backend single post/page/portfolio item: add multiple preview pictures. get a preview picture via ajax request and display it
	...
	//hook into wordpress admin.php
	add_action('wp_ajax_ace_ajax_get_gallery', 'ace_ajax_get_gallery');
}
 ...
 */
if(!function_exists('ace_ajax_get_image_color'))
{
	function ace_ajax_get_image_color()
	{
		#backend single post/page/portfolio item: add multiple preview pictures. get a preview picture via ajax request and display it
		$colorString = "";
		$attachment_id = (int) $_POST['attachment_id'];
		...
		if(function_exists('imagecolorat'))
		{
			...
			$colorString = ace_backend_get_hex_from_rgb($colors['red'],$colors['green'],$colors['blue']);
		}
	...
	//hook into wordpress admin.php
	add_action('wp_ajax_ace_ajax_get_image_color', 'ace_ajax_get_image_color');
}
 ...
 */
if(!function_exists('ace_ajax_switch_menu_walker'))
{
	function ace_ajax_switch_menu_walker()
	{	
	...
	//hook into wordpress admin.php
	add_action('wp_ajax_ace_ajax_switch_menu_walker', 'ace_ajax_switch_menu_walker');
}
 ...
 */
if(!function_exists('ace_ajax_import_data'))
{
	function ace_ajax_import_data()
	{				
		//check if user is allowed to save and if its his intention with a nonce check
		if(function_exists('check_ajax_referer')) { check_ajax_referer('ace_nonce_import_dummy_data'); }
	...
	//hook into wordpress admin.php
	add_action('wp_ajax_ace_ajax_import_data', 'ace_ajax_import_data');
}
 ...
 */
if(!function_exists('ace_ajax_create_dynamic_options'))
{
	function ace_ajax_create_dynamic_options()
	{
		//check if user is allowed to save and if its his intention with a nonce check
		if(function_exists('check_ajax_referer')) { check_ajax_referer('ace_nonce_save_backend'); }
		$options = new ace_database_set();
		...
		if($_POST['method'] == 'add_option_page')
		{
			...
			if(is_array($result))
			{
				...
				$result = "{ace_ajax_option_page}" .$html->create_container_based_on_slug($result) ."{/ace_ajax_option_page}";
				
				if(isset($_POST['defaul_elements']))
				{	
					$elements = unserialize( base64_decode( $_POST['defaul_elements'] ) );
					
					$result .= "{ace_ajax_element}";
	...
	//hook into wordpress admin.php
	add_action('wp_ajax_ace_ajax_create_dynamic_options', 'ace_ajax_create_dynamic_options');
}
 ...
 */
if(!function_exists('ace_ajax_delete_dynamic_options'))
{
	function ace_ajax_delete_dynamic_options()
	{
		//check if user is allowed to save and if its his intention with a nonce check
		if(function_exists('check_ajax_referer')) { check_ajax_referer('ace_nonce_save_backend'); }
		$options = new ace_database_set();
		...
		die("ace_removed_page");
	}
	
	//hook into wordpress admin.php
	add_action('wp_ajax_ace_ajax_delete_dynamic_options', 'ace_ajax_delete_dynamic_options');
}
 ...
 */
if(!function_exists('ace_ajax_delete_dynamic_element'))
{
	function ace_ajax_delete_dynamic_element()
	{
		//check if user is allowed to save and if its his intention with a nonce check
		if(function_exists('check_ajax_referer')) { check_ajax_referer('ace_nonce_save_backend'); }
		$options = new ace_database_set();
		...
		die('ace_removed_element');
	}
	
	//hook into wordpress admin.php
	add_action('wp_ajax_ace_ajax_delete_dynamic_element', 'ace_ajax_delete_dynamic_element');
}
