...
/**
 * This file holds the ace_adminpages class which is needed to build the ace admin option pages for the wordpress backend
 *
 * @author		Afzaal Ameer
...
if( ! class_exists( 'ace_adminpages' ) )
{
	class ace_adminpages
	{
		/**
		...
		/**
         * ace_adminpages Constructor
         *
         * The constructor sets up the superobject and then hooks into wordpress and creates the option pages based on the $this->ace_superobject->option_page_data array.
         * The method that gets attached to the hook is attach_options_to_menu.
         */
		function ace_adminpages(&$ace_superobject)
		{
		...
		*/
		function add_styles()
		{
			...
			$files = ace_backend_load_scripts_by_folder( ACE_CSS );
			...
			{ 
				...
				{
					$filename = basename($file_info['basename'], ".".$file_info['extension']) ;
					wp_enqueue_style($filename , ACE_CSS_URL . $file); 
				}
			}
		}
		...
		*
		* Sorts the $this->ace_superobject->option_page_data array based on index, 
		...
		{	
			...
			foreach( $this->ace_superobject->option_pages as $key => $data_set )
			{
				...
				if($key === 0)
				{
					...
					$ace_page = $page_creation_method(	$this->ace_superobject->base_data['prefix'], 	// page title
														$this->ace_superobject->base_data['prefix'], 	// menu title
				...
				{
					$ace_page = add_submenu_page(	$top_level,								// parent page slug to attach
				...
				if(!empty($ace_page))
				{
					//add scripts and styles to all ace options pages
					add_action('admin_print_scripts-' . $ace_page, array(&$this, 'add_scripts'));
					add_action('admin_print_styles-'  . $ace_page, array(&$this, 'add_styles'));
				}
			}
		}
		...
		* It checks the current page slug and based on that slug filters the $this->ace_superobject->option_page_data options array.
		* All option sets with the same slug get renderd with the help of the ace_htmlhelper class.
		*/
		function render_page()
		{
			...
			$firstClass = 'ace_active_container';
			...
			foreach( $this->ace_superobject->option_pages as $key => $data_set )
			{
				...
				{
					$this->ace_superobject->currentpage = $data_set['title'];
					break;
				}
			}
			

			$this->ace_superobject->page_slug = $current_slug;
			$html = new ace_htmlhelper($this->ace_superobject);
			
			echo $html->page_header();
			
			foreach($this->ace_superobject->option_pages as $option_page)
			{
