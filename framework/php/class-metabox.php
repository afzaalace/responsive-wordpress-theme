...
if( !class_exists( 'ace_meta_box' ) )
{
	...
	class ace_meta_box
	{
		...
		/**
		 * Object of class ace_htmlhelper, necessary to render the different elements
		 * @var obj
		 */
		var $html;
		 ...
		 */
		function ace_meta_box($ace_superobject)
		{	
			
			if(basename( $_SERVER['PHP_SELF']) == "post-new.php" 
			|| basename( $_SERVER['PHP_SELF']) == "post.php")
			{	
				$this->superobject = $ace_superobject;
				$this->html = new ace_htmlhelper($ace_superobject);
				$this->html->context = "metabox";
				$this->meta_prefix = ace_backend_safe_string($ace_superobject->base_data['prefix']);
				...
				{				
					...
					{	
						...
						add_meta_box( 	
							...
							array('ace_current_box'=>$box) 	// callback arguments so we know which box we are in
						);  
					}
				}
			}
		...
		{	
			global $post;
			$output = "";
			$box = $metabox['args']['ace_current_box'];
			
			if(!is_object($post)) return;
			
			$custom_fields = get_post_meta($post->ID, '_ace_elements_'.$this->superobject->option_prefix, true);
			$custom_fields = apply_filters('ace_meta_box_filter_custom_fields', $custom_fields, $post->ID);
			...
			{	
				if($element['slug'] == $box['id'])
				{
					...
					{	
						...
						$output .= '<div class="ace_meta_box ace_meta_box_'.$element['type'].' meta_box_'.$box['context'].'">';
		...
		function save_post()
		{
			if(isset($_POST['post_ID']))
			{
				...
				if(function_exists('check_ajax_referer')) { check_ajax_referer('ace_nonce_save_metabox','ace-nonce'); }
				...
				$result = ace_ajax_save_options_create_array($meta_array, true);
				update_post_meta($post_id , '_ace_elements_'.$this->superobject->option_prefix, $result);
				
				//also save the data to a neutral field that can be used for compatibility with other themes
				update_post_meta($post_id , '_ace_elements_theme_compatibility_mode', $result);
				...
				do_action('ace_meta_box_save_post', $post_id, $result);
			}
		}
	}
}
