 ...
 * ACE HTML HELPER
 *
 * This class receives an extended $ace_superobject file when called, which holds the information which page we are currently viewing
 * Based on that information it renders all form elements necessary for creating the option page.
 * Methods can also be called without looping over the $ace_superobject, but for html generating purposes in other parts of the theme
 * (for example meta boxes, widgets etc)
 * @package AceFramework
 * 
 */
 
if( ! class_exists( 'ace_htmlhelper' ) )
{
	class ace_htmlhelper
	{
		/**
		 * This object holds the $ace_superobject with all the previously stored informations like theme/plugin data, options data, default values etc
		 * @var obj
		 */
		var $ace_superobject;
		
		/**
		 * This object holds the ace_database_set controller methods to check if an item is grouped or not
		 * @var obj
		 */
		var $set;
         ...
         */
		function ace_htmlhelper($ace_superobject = false)
		{
		
			if(!$ace_superobject) { $ace_superobject = $GLOBALS['ace']; }
			$this->ace_superobject = $ace_superobject;
			
			$options = get_option($this->ace_superobject->option_prefix);

			//check which option pages were already saved yet and need replacement of the default values
			foreach($ace_superobject->option_pages as $page)
			{
		...
		{
			$page_elements = array();
			if(isset($this->ace_superobject->option_page_data))
			{
				foreach($this->ace_superobject->option_page_data as $key => $value)
				{
					if($value['slug'] == $slug)
					{
		...
		function create_container_based_on_slug($option_page, $firstClass = '')
		{
			...
			{
				$output .= "<a href='#".$option_page['slug']."' title='".$option_page['removable']."' class='ace_remove_dynamic_page'>".$option_page['removable']."</a>";
			}
		...
		function render_single_element( $element )
		{
			...
			if( method_exists( $this, $element['type'] ) )
			{
				...
				{
					if(isset($this->ace_superobject->page_slug) && 
					   isset($this->replace_default[$element['slug']]) && 
					   isset($this->ace_superobject->options[$this->ace_superobject->page_slug][$element['id']]))
					{
						$element['std'] = $this->ace_superobject->options[$this->ace_superobject->page_slug][$element['id']];
					}
				}
				...
				if($element['dynamic'])
				{
					$output .= '<div class="ace_row">';
					$output .= '	<div class="ace_style_wrap ace_style_wrap_portlet">';
					$output .= '		<div class="ace-row-portlet-header">'.$element['name'].'<a href="#" class="ace-item-edit">+</a></div>';
					$output .= '		<div class="ace-portlet-content">';
					$output .= '		<span class="ace_clone_loading ace_removable_element_loading ace_hidden">Loading</span>';
					$output .= "		<a href='#".$element['id']."' title='".$element['removable']."' class='ace_remove_dynamic_element'><span>".$element['removable']."</span></a>";
					$dynamic_end = '<div class="ace_clear"></div></div></div></div>';
				}				
		...
		{	
			...
			if(!empty($element['attachment-prefix']))
			{
				...
				$gallery_mode = true;
				$postId = ace_media::get_custom_post($element['attachment-prefix'].$element['std']);
			}
			else
			{
				$postId = ace_media::get_custom_post($element['name']);
			}
			...
			//video or image, advanced or default upload?
				...
				{
					$mode = ' ace_advanced_upload';
					
					if(!is_numeric($element['std']) && $element['std'] != '')
					{
						$prevImg = '<a href="#" class="ace_remove_image">remove</a><img src="'.ACE_IMG_URL.'icons/video.png" alt="" />';
					}
					else if($element['std'] != '')
					{
						$prevImg = '<a href="#" class="ace_remove_image">remove</a>'.wp_get_attachment_image($element['std'], array(100,100));
					}
				...
				else
				{
					...
					{
						$prevImg = '<a href="#" class="ace_remove_image">remove</a><img src="'.$element['std'].'" alt="" />'; 
					}
				}
			...
			if($gallery_mode)
			{
				...
				{
					$image_url_array[] = ace_image_by_id($attachment->ID, array('width'=>80,'height'=>80));
				}
				
				$output  .= "<div class='ace_thumbnail_container'>";
				
				if(isset($image_url_array[0]))
				{
					foreach($image_url_array as $key => $img) 
					{
						$output  .= "<div class='ace_gallery_thumb'><div class='ace_gallery_thumb_inner'>".$img."</div></div>";
					}
			...
			$output .= '<div class="ace_upload_container ace_upload_container_'.$postId.$mode.'">';
			$output .= '	<span class="ace_style_wrap ace_upload_style_wrap">';
			

			$output .= '	<input type="text" class="ace_upload_input '.$element['class'].'" value="'.$element['std'].'" name="'.$element['id'].'" id="'.$element['id'].'" />';
			$output .= '	<a href="#'.$postId.'" class="ace_button ace_uploader" title="'.$element['name'].'" id="ace_upload'.$element['id'].'">'.$element['button-label'].'</a>';
			$output .= '	</span>';
			$output .= '	<div class="ace_preview_pic" id="div_'.$element['id'].'">'.$prevImg.'</div>';
			$output .= '	<input class="ace_upload_insert_label" type="hidden" value="'.$element['label'].'" />';
			if($gallery_mode) $output .= '	<input class="ace_gallery_mode" type="hidden" value="'.$postId.'" />';
			$output .= '</div>';
		...
		function upload_gallery( $element )
		{
			...
			for ($i = 0; $i < $iterations; $i++)
			{
				...
				$sub_output  .= '<div class="ace_set ace_row '.$element['class'].'" id="ace_'.$element['id'].'" >';
				$sub_output  .= 	'<div class="ace_single_set"><div class="ace_handle"></div>';
				$sub_output	 .= 		$this->get_subelements($element, $i);
				$sub_output  .= '		<a class="ace_remove_set remove_all_allowed" href="#">'.__('(remove)').'</a>';
			...
			$postId = $post_ID; //ace_media::get_custom_post($element['name']);
			$output = "";
			
			$output .= '<div class="ace_gallery_upload_container ace_gallery_upload_container'.$postId.'">';
			$output .= '<div class="ace_sortable_gallery_container">';
			...
			$output .= '	<span class="ace_style_wrap ace_upload_style_wrap">';
				//generate the upload link
					$output .= '<a href="#" class="ace_button ace_gallery_uploader" title="'.$element['name'].'" id="ace_gallery_uploader '.$element['id'].'"';
			...
			$output .= '	<span class="ace_style_wrap ace_upload_style_wrap">';
				//generate the upload link
					$output .= '<a href="#" class="ace_button ace_gallery_uploader" title="'.$element['name'].'" id="ace_gallery_uploader '.$element['id'].'"';
					$output .= 'data-label="'.$element['label'].'" data-video-insert = "ace_video_insert"';
					$output .= 'data-attach-to-post = "'.$postId.'" ';
					$output .= 'data-real-id="'.$real_id.'" ';
					$output .= 'data-this-id="'.$element['id'].'" ';
		...
		{	
			...
			{
				$autodetect = '<a id="ace_autodetect_'.$element['id'].'" class="ace_button ace_autodetect" href="#'.$element['autodetect'].'">Auto detection</a><span class="ace_loading"></span>';
				$autodetectClass = ' ace_auto_detector';
			}
			
				
			$output  = '<span class="ace_style_wrap ace_colorpicker_style_wrap'.$autodetectClass.'">';
			$output .= '<input type="text" class="ace_color_picker '.$element['class'].'" value="'.$element['std'].'" id="'.$element['id'].'" name="'.$element['id'].'"/>';
			$output .= '<span class="ace_color_picker_div"></span>'.$autodetect.'</span>';
			return $output;
		}
		...
		{	
			...
			$output  = '<div class="ace_hr '.$class.'"><div class="ace_inner_hr"></div></div>';
			return $output;
		}
		...
		{	
			$output = "";
			$nonce	 = 	wp_create_nonce ('ace_nonce_import_dummy_data');
			$output .= '<input type="hidden" name="ace-nonce-import" value="'.$nonce.'" />';
			$output .= '<span class="ace_style_wrap"><a href="#" class="ace_button ace_import_button">Import dummy data</a></span>';
			$output .= '<span class="ace_loading ace_import_loading"></span>';
			$output .= '<div class="ace_import_wait"><strong>Import started.</strong><br/>Please wait a few seconds and dont reload the page. You will be notified as soon as the import has finished! :)</div>';
			$output .= '<div class="ace_import_result"></div>';
			return $output;
		}
		...
		{
			$output = '<span class="ace_style_wrap"><a href="#" class="ace_button ace_button_grey ace_reset">Reset all options</a></span>';
			return $output;
		}
	...
	function dynamical_add_elements($element)
	{
		...
		$output .= '<div class="ace_dynamical_add_elements_container">';
		$output .= '	<span class="ace_style_wrap ace_dynamical_add_elements_style_wrap">';
		$output .= '<span class="ace_select_unify"><select class="'.$element['class'].' ace_dynamical_add_elements_select">';
		$output .= '<option value="">Select Element</option>  ';
	

		//gets the option array with available elements
		if(! @include(ACE_BASE.$element['options_file']))
		{
		...
		$output .= '</select>';
		$output .= '<span class="ace_select_fake_val">Select Element</span></span>';
		$output .= '	<a href="#" class="ace_button ace_dynamical_add_elements" title="'.$element['name'].'" id="ace_'.$element['id'].'">Add Element</a>';
		$output .= '	<span class="ace_loading ace_beside_button_loading"></span>';
		$output .= '	</span>';
		$output .= '	<input class="ace_dynamical_add_elements_parent" type="hidden" value="'.$element['slug'].'" />';
		$output .= '	<input class="ace_dynamical_add_elements_config_file" type="hidden" value="'.$element['options_file'].'" />';
