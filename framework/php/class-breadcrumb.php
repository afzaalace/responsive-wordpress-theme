...
if(!class_exists('ace_breadcrumb'))
{
	class ace_breadcrumb
	{
		var $options;
		
	function ace_breadcrumb($options = ""){
		...
		global $post;
		echo '<p class="breadcrumb"><span class="breadcrumb_info">'.__('You are here:','ace_framework').'</span> <a href="'.get_bloginfo('url').'">';
			bloginfo('name');
			echo "</a>";
		...
		if(is_404()){ return _e("404 - Page not Found",'ace_framework'); }
		
		if(is_search()){ return _e("Search",'ace_framework');}	
...
/*-----------------------------------------------------------------------------------*/
/* ace_breadcrumbs() - Custom breadcrumb generator function  */
/*
/* Params:
/*
/* Arguments Array:
...
function ace_breadcrumbs( $args = array() ) {
	global $wp_query, $wp_rewrite;

	/* Get the textdomain. */
	$textdomain = 'ace_framework';
	...
	$args = apply_filters( 'ace_breadcrumbs_args', $args );
	...
	elseif ( is_home() ) {
		...
		$trail = array_merge( $trail, ace_breadcrumbs_get_parents( $home_page->post_parent, '' ) );
	...
	elseif ( is_singular() ) {
		...
		if ( 'page' !== $post_type && 'post' !== $post_type ) {
			...
			if ( !empty( $path ) )
				$trail = array_merge( $trail, ace_breadcrumbs_get_parents( '', $path ) );
		...
		*/
		...
		if ( empty( $path ) && 0 !== $parent || 'attachment' == $post_type )
			$trail = array_merge( $trail, ace_breadcrumbs_get_parents( $parent, '' ) );
		...
		if ( 'post' == $post_type && $show_posts_page == true && 'page' == get_option( 'show_on_front' ) ) {
			...
			if ( $posts_page != '' && is_numeric( $posts_page ) ) {
				$trail = array_merge( $trail, ace_breadcrumbs_get_parents( $posts_page, '' ) );
			}
		}
	...
	elseif ( is_archive() ) {
		...
		if ( is_tax() || is_category() || is_tag() ) {
			...
			if ( is_taxonomy_hierarchical( $term->taxonomy ) && $term->parent )
				$trail = array_merge( $trail, ace_breadcrumbs_get_term_parents( $term->parent, $term->taxonomy ) );
		...
		elseif ( is_date() ) {
			...
			if ( $wp_rewrite->front )
				$trail = array_merge( $trail, ace_breadcrumbs_get_parents( '', $wp_rewrite->front ) );
	...
	$trail = apply_filters( 'ace_breadcrumbs_trail', $trail, $args );

	/* Connect the breadcrumb trail if there are items in the trail. */
	if ( is_array( $trail ) ) {

		/* Open the breadcrumb trail containers. */
		$breadcrumb = '<div class="breadcrumb breadcrumbs ace-breadcrumbs"><div class="breadcrumb-trail">';
	...
	$breadcrumb = apply_filters( 'ace_breadcrumbs', $breadcrumb );
...
/*-----------------------------------------------------------------------------------*/
/* ace_breadcrumbs_get_parents() - Retrieve the parents of the current page/post */
/*-----------------------------------------------------------------------------------*/
/**
 ...
 */
function ace_breadcrumbs_get_parents( $post_id = '', $path = '' ) {
...
} // End ace_breadcrumbs_get_parents()

/*-----------------------------------------------------------------------------------*/
/* ace_breadcrumbs_get_term_parents() - Retrieve the parents of the current term */
/*-----------------------------------------------------------------------------------*/
/**
 ...
 */
function ace_breadcrumbs_get_term_parents( $parent_id = '', $taxonomy = '' ) {
...
} // End ace_breadcrumbs_get_term_parents()