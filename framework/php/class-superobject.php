...
/**
 * This file holds the ace_superobject class which is the core of the framework
 *
 * @author		Afzaal Ameer
 ...
 * The class is responsible for loading the options data and adding it to the $ace superobject.
 *
 * The class only gets loaded if it wasnt already defined by a Wordpress Plugin based on the Ace Plugin Framework which uses a similar function set
 *  
 * @package AceFramework
 * 
 */

if( ! class_exists( 'ace_superobject' ) )
{
	class ace_superobject
	{
		...
		/**
		 * option pages retrieved from the config files in theme_option_pages, used to create the ace admin options panel.
		 * @var array
		 */
		var $option_pages = array();
		
		/**
		 * option page data retrieved from the config files in theme_option_pages, used to create the items at the ace admin options panel.
		 * @var array
		 */
		var $option_page_data = array();
		
		/**
		 * This object holds the ace style informations for php generated styles in the backend
		 * @var obj
		 */
		var $style;
         ...
         */
		public function ace_superobject( $base_data )
		{	
			
			$this->base_data = $base_data;
			$this->option_prefix = 'ace_options_'.ace_backend_safe_string( $this->base_data['prefix'] );
			
			//set option array
			$this->_create_option_arrays();
			
			if(current_theme_supports( 'ace_mega_menu' ) ) { new ace_megamenu($this); }
			
			$this->style = new ace_style_generator($this);
			...
			if( is_admin() ) 
			{
				...
				new ace_adminpages($this);
				new ace_meta_box($this);
				new ace_wp_export($this);
			}
		...
		private function _create_option_arrays()
		{
			...
			include(ACE_BASE.'/includes/admin/register-admin-options.php');
			if(isset($ace_pages)) $this->option_pages = $ace_pages;
			if(isset($ace_elements)) $this->option_page_data = $ace_elements;
			 ...
			 */
			$this->options = apply_filters( 'ace_filter_global_options', $database_option );
		...
		/**
         * This function is executed when the admin header is printed and will add the ace_framework_globals to javascript 
         * The ace_framework_globals object contains information about the framework
         */
		function set_javascript_framework_url()
		{
			echo "\n <script type='text/javascript'>\n /* <![CDATA[ */  \n";
			echo "var ace_framework_globals = {\n \tframeworkUrl: '".ACE_FW_URL."', \n \tinstalledAt: '".ACE_BASE_URL."'\n \t}; \n /* ]]> */ \n ";
			echo "</script>\n \n ";
		}
