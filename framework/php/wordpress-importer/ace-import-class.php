<?php
class ace_wp_import extends WP_Import
{
	var $preStringOption; 
	var $results;
	var $getOptions;
	...
	{	
		...
		if(is_array($options))
		{
			...
			{
				$database_option[$page['parent']] = $this->extract_default_values($options[$page['parent']], $page, $ace->subpages);
			}
		}
		
		if(!empty($database_option))
		{
			update_option($ace->option_prefix, $database_option);
		}
		
		if(!empty($dynamic_pages))
		{
			update_option($ace->option_prefix.'_dynamic_pages', $dynamic_pages);
		}
		
		if(!empty($dynamic_elements))
		{
			update_option($ace->option_prefix.'_dynamic_elements', $dynamic_elements);
		}
	}
	...
	function set_menus()
	{
		...
		$ace_menus  = wp_get_nav_menus();
		
		if(!empty($ace_menus) && !empty($ace_config['nav_menus']))
		{
			foreach($ace_menus as $ace_menu)
			{
				//check if we got a menu that corresponds to the Menu name array ($ace_config['nav_menus']) we have set in functions.php
				if(is_object($ace_menu) && in_array($ace_menu->name, $ace_config['nav_menus']))
				{
					$key = array_search($ace_menu->name, $ace_config['nav_menus']);
					if($key)
					{
						//if we have found a menu with the correct menu name apply the id to the menu location
						$locations[$key] = $ace_menu->term_id;
					}
				}
			}
		}
