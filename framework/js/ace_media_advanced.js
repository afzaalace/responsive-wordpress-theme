/**
 * This file holds the main javascript functions needed for new version of the ace-media uploads with improved sorting options
 *
 * @author		Afzaal Ameer
...
(function($)
{
	var ace_media_advanced = {
		...
		bind_click: function()
		{
			...
			$('.ace_gallery_uploader').live('click', function()
			{
				//collect current link properties
				var element 		= $(this),
					container		= element.parents('.ace_gallery_upload_container:eq(0)'),
					set				= element.parents('.ace_set:eq(0)'),
					insertContainer = container.find('.ace_sortable_gallery_container'),
					title  			= this.title,
					...
					value_field		= set.find('.ace_gallery_image_value'),
					image_field		= value_field.parents('.ace_gallery_image:eq(0)'),
					meta_active    	= $('input[name=meta_active]', '#ace_hidden_data'),
				...
				var uri_string  = 'media-upload.php?post_id='+attach_to_post;
					uri_string += '&amp;ace_gallery_active=true';
		if(video)	uri_string += '&amp;tab='+video+'&amp;height=300';
					uri_string += '&amp;ace_gallery_label='+encodeURI(label),
		...
		if(video) $('body').addClass(video + '_height_mod height_mod_active');
				...
				ace_media_advanced.window.ace_framework_globals.gallery_editor = 
				{
					element: 		element,
		...
		insert_click: function()
		{
			...
			$('.ace_send_to_gallery').live('click', function()
			{
				var link 			= $(this),
					attachment_id 	= link.data('attachment-id'),
					loading			= link.next('.ace_gallery_loading').css({visibility:'visible'})
																		.removeClass('ace_loading_done ace_loading_error');
				
				
				if(ace_media_advanced.window.ace_framework_globals.gallery_editor &&
				   ace_media_advanced.window.ace_framework_globals.gallery_editor.overwrite)
				{	
					ace_media_advanced.replace_image(attachment_id, loading);
				}
				else
				{
					ace_media_advanced.request_image(attachment_id, loading);
				}
				return false;
			});
		},
		...
		insert_inline_content_click: function(button, content_field, close)
		{
			...
			var loading = $('<div class="ace_gallery_loading"></div>').insertAfter(button);
			...
			button.click(function()
			{
				...
				if(content != "")
				{
					...
					ace_media_advanced.request_image(content, loading, data, close);
				}
		...
		open_close_click: function()
		{
			...
			{
				var el = $(this),
					parent = el.parents('.ace_set:eq(0)');
		...
		get_gallery_settings: function()
		{
			if( ace_media_advanced.window.ace_framework_globals.gallery_editor )
			{
				return ace_media_advanced.window.ace_framework_globals.gallery_editor;
			}
			//else
			return false;
		},
		...
		{
			ace_media_advanced.window.ace_framework_globals.gallery_editor = false;
		},
		
		//clone the insert button and display it at the top of each item
		clone_insert_button: function()
		{
			if(ace_media_advanced.window.ace_framework_globals.gallery_editor == 'undefined') return false;
			...
			if(container.length)
			{
				...
				media_items.each(function()
				{
					var current 	= $(this).addClass('button_cloned'),
						...
						button 		= current.find('.ace_send_to_gallery').clone(true).prependTo(filename),
						loading		= $('<div class="ace_gallery_loading"></div>').insertAfter(button);
		...
		add_insert_all_button: function()
		{
			var gallery 	= $('#gallery-form'),
				gallery_data = ace_media_advanced.window.ace_framework_globals.gallery_editor;
			
			if(gallery.length && gallery_data && !gallery_data.overwrite)
			{
			var submit = $('.ml-submit:eq(0)'),
				update_gal = $('<input type="submit" id="ace_insert_all" class="button savebutton" value="Add all Images to Slideshow"/>'),
				loading 	= $('<div class="ace_gallery_loading ace_gallery_loading_all"></div>')		
							
				update_gal.appendTo(submit).bind('click', function()
				{
					var data = {};
					data.activate_filter = 'ace_ajax_fetch_all';
					loading.css({visibility:'visible'}).removeClass('ace_loading_done ace_loading_error');
					ace_media_advanced.request_image( '' ,loading, data);
					return false;
				});
		...
		overwrite_default_uploader: function()
		{
			...
			window.send_to_editor = function(html)
     		{     
				...
				if(ace_media_advanced.get_gallery_settings() == false)
				{
					window.orig_send_to_editor(html);
				}
				else
				{
					ace_media_advanced.clear_gallery();
					ace_media_advanced.window.tb_remove();
				}
     		};
		},
		...
		sortable: function()
		{
			$( ".ace_handle" ).disableSelection();	
		
			$('.ace_sortable_gallery_container').sortable({
		...
		modify_filter_url: function()
		{
			...
			{
				//duplication check
				var labelInsert	 = filter.find("input[name=ace_gallery_label]"),
					galleryInsert= filter.find("input[name=ace_gallery_active]");
				
				if(gallery_data && gallery_data.label && !labelInsert.length)
				{
					filter.prepend("<input type='hidden' name='ace_gallery_label' value='"+gallery_data.label+"'/>");
				}
				
				if(gallery_data && !galleryInsert.length)
				{
					filter.prepend("<input type='hidden' name='ace_gallery_active' value='true'/>");
				}
			}
		},
		...
		{	
			if(!scope || scope == 'undefined') scope = '.ace_sortable_gallery_container';
			var container = $(scope);
			
			container.each(function()
			{
				var current_container = $(this),
					sets = current_container.find('.ace_set');
					
				if(!sets.length)  sets = current_container.filter('.ace_set');
				
				sets.each(function()
				{
					var current_set = $(this),
						tabs 		= current_set.find('.ace_tab'),
						title_group = $('<div class="tab-title-container"></div>').prependTo(current_set.find('.ace_visual_set:eq(0)'));
						
						tabs.each(function(i){
						
							var current_tab = $(this),
								title		= current_tab.data('group-name')
								active 		= 'ace_active_tab_title';
							...
							.click(function(){
								...
								tabs.css({display:'none'}).removeClass('ace_active_tab');
								current_tab.css({display:'block'}).addClass('ace_active_tab');
								
								title_group.find('a').removeClass('ace_active_tab_title');
								$(this).addClass('ace_active_tab_title');
		...
		{			
			...
			{
				var gallery_data = ace_media_advanced.window.ace_framework_globals.gallery_editor;
		...
		{	
			...
			$.ajax({
	 		  ...
	 		  {
	 		  	loading.addClass('ace_loading_done');
	 		  	...
	 		  	{
					gallery_data.image_field.find('a').html('<img src="'+ace_framework_globals.frameworkUrl+'images/icons/video.png" alt="" />');
	 		  	}
		...
		request_image: function(attachment_id, loading, data_passed, close)
		{
			...
			if(!gallery_data) 
			{
				loading.addClass('ace_loading_error');
				return;
			}
			...
			$.ajax({
				...
				success: function(response)
				{
					...
					{	
						...
						{
							if(close == true) setTimeout( ace_media_advanced.window.tb_remove, 500);
						}
	...
	$(function()
	{
		...
		//ace_media_advanced.overwrite_default_uploader(); //currently not necessary!
		
		//overwrite the thickbox close function
		ace_media_advanced.overwrite_thickbox_close();
		
		//sorting possible
		ace_media_advanced.sortable();
		
		//modify search in case user uses filter
		ace_media_advanced.modify_filter_url();
		
		//bind click on upload button
		ace_media_advanced.bind_click();
		
		//bind click on upload button
		ace_media_advanced.insert_click();
		
		//bind click on upload button
		ace_media_advanced.insert_inline_content_click('#ace_insert_video', '#src', true);
		
		//bind click on upload button
		ace_media_advanced.open_close_click();
		
		//activate tabs + tab functionality that is made out of visual groups
		ace_media_advanced.activate_tabs();
		
		//clone the insert button
		ace_media_advanced.clone_insert_button();
		$(".media-item").live('mouseenter',ace_media_advanced.clone_insert_button);
		
		//adds an insert all button 
		ace_media_advanced.add_insert_all_button();
	
 	});
 	
 	
 	$.fn.ace_media_advanced_plugin = function(variables) 
	{	
		ace_media_advanced.activate_tabs(this);
	};


})(jQuery);	 

function ace_log(text) 
{
	var ios = navigator.userAgent.toLowerCase().match(/(iphone|ipod|ipad)/);
	( (window.console && console.log && !ios) || (window.opera && opera.postError && !ios) ||  ace_text_log).call(this, text);
	
	function ace_text_log(text)
	{
		var logfield = jQuery('.ace_logfield');
		if(!logfield.length) logfield = jQuery('<pre class="ace_logfield"></pre>').appendTo('body').css({	zIndex:100000, 
																											padding:"20px", 
		...
		var text = ace_get_object(text);
		logfield.html(val + "\n<br/>" + text);
	}
	
	function ace_get_object(obj)
	{
		var sendreturn = obj;
