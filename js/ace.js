...
//global path: ace_framework_globals.installedAt
...
jQuery(document).ready(function(){
	...
	if(jQuery.fn.adaptace)	
	jQuery('.adaptace').adaptace();
	
	if(jQuery.fn.ace_base_slider)
	{	
		jQuery('.fade_slider').ace_base_slider({transition:'fade'});
		jQuery('.move_slider').ace_base_slider();
		jQuery('.slideshow').ace_base_control_hide();
		jQuery('.slideshow').ace_external_controls();
	}
	

	//activates the mega menu javascript
	if(jQuery.fn.aceMegamenu)		
	jQuery(".main_menu .ace_mega, .sub_menu>ul, .sub_menu>div>ul").aceMegamenu({modify_position:true});

	//activates the prettyphoto lightbox
	if(jQuery.fn.ace_activate_lightbox)		
	jQuery('body').ace_activate_lightbox();
	
	//activates the hover effect for image links
	if(jQuery.fn.ace_activate_hover_effect)
	jQuery('body').ace_activate_hover_effect();

	
	// enhances contact form with ajax capabilities
	if(jQuery.fn.ace_ajax_form)
	jQuery('.ajax_form').ace_ajax_form();
	
	//smooth scrooling
	if(jQuery.fn.ace_smoothscroll)
	jQuery('a[href*=#]').ace_smoothscroll();
	
	//activates the shortcode content slider
	if(jQuery.fn.ace_sc_slider)
	{								
		jQuery(".content_slider").ace_sc_slider({appendControlls:{}});
		jQuery(".shop_slider_yes ul").ace_sc_slider({appendControlls:false, group:true, slide:'.product', arrowControll: true, autorotationInterval:'parent'});
	}
	//activates the toggle shortcode
	if(jQuery.fn.ace_sc_toggle)
	jQuery('.togglecontainer').ace_sc_toggle();
	
	//activates the tabs shortcode
	if(jQuery.fn.ace_sc_tabs)
	jQuery('.tabcontainer').ace_sc_tabs();
	
	//activate html5 flare video player
	if(jQuery.fn.ace_video_activation)
	jQuery(".ace_video").ace_video_activation({ratio:'16:9'});
	
	//activates the mega menu javascript
	if(jQuery.fn.ace_menu_helper)		
	jQuery(".main_menu .menu").ace_menu_helper({modify_position:true});
	
	
	//activate html5 flare video player
	if(jQuery.fn.ace_hide_info_text)
	jQuery("#info_text_header").ace_hide_info_text();


	
	// actiavte portfolio sorting
	if(jQuery.fn.ace_iso_sort)
	jQuery('.portfolio-sort-container').ace_iso_sort();
	
	jQuery(window).resize(ace_menu_align).trigger('resize');
	ace_ie_fix();
	ace_more_link_fade('.template-portfolio-overview:not(.portfolio-size-1)');
	
	
	// improves comment forms
	if(jQuery.fn.ace_empty_input)
	jQuery('#s, #search-fail input').ace_empty_input();
...
function ace_more_link_fade(container)
{
...
(function($)
{
	$.fn.aceMegamenu = function(variables) 
	{
		var defaults = 
		{
...
(function($)
{
	$.fn.ace_iso_sort = function(options) 
	{
		...
		{	
			...
			$(window).bind('ace_images_loaded', function()
			{
				setTimeout(function(){
...
(function($)
{
	$.fn.ace_empty_input = function(options) 
	{	
		return this.each(function()
		{	
...
(function($)
{
	$.fn.ace_menu_helper = function(variables) 
	{
		var defaults = 
		{
			modify_position:true,
...
(function($)
{
	$.fn.ace_video_activation = function(options) 
	{
		var defaults = 
		{
			ratio: '16:9'
		...
		return this.each(function()
		{
		    ...
		    fv.width(fv_width);
		     ...
		     projekktor(id_to_apply, {
				 ...
				 debug: false, //'console',
				 controls: true,
				 playerFlashMP4: ace_framework_globals.installedAt + 'js/projekktor/jarisplayer.swf',
				 height: fv_height,
				 width: fv_width,
				 poster: posterImg
			});
...
(function($)
{
	$.fn.ace_sc_tabs= function(options) 
	{
		var defaults = 
		{
			heading: '.tab',
...
(function($)
{
	$.fn.ace_sc_toggle = function(options) 
	{
		var defaults = 
		{
			heading: '.toggler',
...
(function($)
{
	$.fn.ace_smoothscroll = function(variables) 
	{
		return this.each(function()
		{
			$(this).click(function() {
...
(function($)
{
	$.fn.ace_hide_info_text = function(variables) 
	{
		return this.each(function()
		{
			var container 	= $(this),
...
(function($)
{
	$.fn.ace_activate_lightbox = function(variables) 
	{
		var defaults = 
		{
...
(function($)
{
	$.fn.ace_activate_hover_effect = function(variables) 
	{
		var defaults = 
		{
			autolinkElements: 'a[rel^="prettyPhoto"], a[rel^="lightbox"], a[href$=jpg], a[href$=png], a[href$=gif], a[href$=jpeg], a[href$=".mov"] , a[href$=".swf"] , a[href*="vimeo.com"] , a[href*="youtube.com"], a.external-link, .ace_mega a, .dynamic_template_columns a'
		};
		...
		{	
			$(options.autolinkElements, this).not(".noLightbox a").contents('img').each(function()
			{
				...
				if(a.is('a'))
				{
					...
					if(img.is('.ace_mega img')) a.css({position:'relative', display:'inline-block'}); 
...
(function($)
{
	$.fn.ace_sc_slider = function(variables, callback) 
	{
		...
		return this.each(function()
		{
			...
			container.methods = 
			{
				...
				preload: function()
				{
					...
					{
						container.aceImagePreloader(container.methods.init);
					}
				},
...
(function($)
{
	$.fn.ace_ajax_form = function(variables) 
	{
		var defaults = 
		{
			sendPath: 'send.php',
