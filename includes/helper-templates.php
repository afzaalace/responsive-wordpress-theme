...
class ace_dynamic_template
{
	...
	function ace_dynamic_template($template_name)
	{
		global $ace, $ace_config;
		$this->post_id = ace_get_the_ID();
		$this->template_name = $template_name;
		$this->template_elements = $this -> _get_template_elements();
		$ace_config['dynamic_template'] = true;
	}
	...
	*/
	function _get_template_elements()
	{
		...
		if(isset($ace->options['templates']))
		{
			$ace->options['templates'] = ace_deep_decode($ace->options['templates']);
		}

		foreach($ace->option_page_data as $key => $element)
		{
			if($element['slug'] == $this->template_name && isset($element['dynamic']))
			{
				//save the saved option into the element array
				if(isset($ace->options['templates'][$element['id']]))
				{
					$ace->option_page_data[$key]['saved_value'] = $ace->options['templates'][$element['id']];
				}
				
				$template_elements[] = $ace->option_page_data[$key];
			}
		}
	...
	*/
	function get_option($option_name)
	{
		global $ace;
		$option = false;
		if(isset($ace->options['templates']) && isset($ace->options['templates'][$this->template_name.$option_name]))
		{
			$option = ace_deep_decode($ace->options['templates'][$this->template_name.$option_name]);
		}
		return $option;
	}
	...
	function set_layout()
	{
		global $ace_config;
		 ...
		 //retrieve page layout: get global option for the dynamic template, then overwrite in case the default layout was changed in the page edit screen
	 	 $ace_config['layout'] = $this->get_option('dynamic_page_layout');
		 ace_template_set_page_layout(false, $ace_config['layout']);
		 $this->layout = $ace_config['layout'];
	}
	...
	*/
	...
	/**
	* This function creates the html code necessary for a slideshow. It uses the ace_slideshow class located in includes/helper-slideshow.php to do that
	*
	...
	*/
	function slideshow($element)
	{
		...
		$id 	= $dynamic_slideshow_which_post_page == 'self' ? ace_get_the_ID() : $dynamic_slideshow_page_id;
		$type 	= ace_post_meta($this->post_id, '_slideshow_position');
		
		
		$slider = new ace_slideshow($id);
	...
	* The output of the function is passed to the $ace_config['slide_output'] (which is displayed in header.php), instead of echoing it or saving it to the default ouput array.
	...
	*/
	function special_slider_config()
	{
		...
		{
			$ace_config['slide_output']  = $this -> get(0, false);
			
			//check if we are at a fullwidth page
			if(isset($ace_config['slide_output']) && $type == 'big')
			{
				$this->unset_key(0);
			}
			else
			{
				$ace_config['slide_output'] = "";
			}
		}
		...
		if($this->check(0) == 'textarea' && $this->check(1) == 'slideshow')
		{
			...
			$ace_config['slide_output'] = $this -> get(1, false);
			
			if($ace_config['slide_output']) $ace_config['slide_output']  = $this -> get(0, false) . $ace_config['slide_output'];
			
			if($ace_config['slide_output'] && $ace_config['layout'] == 'fullsize' )
			{
				$this->unset_key(0);
				$this->unset_key(1);
			}
	...
	*/
	...
	function hr($element)
	{
		...
		{
			case 'default': 		$output .= ace_flag(false, $extraClass); break;
			case 'default_small': 	$output .= ace_flag(false, 'supersmall '.$extraClass); break;
			case 'whitespace': 		$output .= "<div class='hr hr_invisible $extraClass'></div>"; break;
			case 'custom': 			$output .= ace_flag($element['saved_value'][0]['dynamic_hr_text'],$extraClass); break;
		}
	...
	*/
	...
	{	
		...
		{
			case 'self': 		$heading = get_the_title(ace_get_the_ID()); break;
	...
	function textarea($element)
	{
		...
		switch($dynamic_text_styling)
		{
			...
			case 'callout': 	$output .= "<div class='outer_callout $extraClass'>";
								...
								{
									$output .= "<a class='big_button ace-button button' href='".ace_get_link($element['saved_value'][0], 'dynamic_text_button_')."'>".$dynamic_text_button."</a>";
	...
	*/
	...
	function blog($element)
	{
		...
		$ace_config['new_query'] = "posts_per_page=".$dynamic_blog_posts_per_page."&paged=".get_query_var( 'paged' );
		
		if(!isset($dynamic_blog_cats)) $dynamic_blog_cats = "";
		
		if($dynamic_blog_cats != 'null' && $dynamic_blog_cats != '')
		{
			$ace_config['new_query'] .= '&cat='.$dynamic_blog_cats;
		}
		
		if($dynamic_blog_pagination != 'yes')
		{
			$ace_config['remove_pagination'] = true;
		}
		

		$output = "";
		$temp_layout = $ace_config['layout'];
		$more = 0;

		//'sidebar_class|three : content_class|nine alpha : layout|sidebar_right'
		if($ace_config['layout'] == 'fullsize')
		{
			$fulwidth_replace = true;
			$temp_layout = $ace_config['layout'];
			$temp_content = $ace_config['content_class'];
			$temp_sb = $ace_config['sidebar_class'];
			
			ace_template_set_page_layout(false, ace_get_option('blog_layout'));
		}
		
		$extraClass = "dynamic_el_".$this->dynamic_counter;
		
		ob_start(); //start buffering the output instead of echoing it
		echo "<div class='template-blog ".$ace_config['layout']." $extraClass'>";
		echo "<div class='content units ".$ace_config['content_class']."'>";
		get_template_part( 'includes/loop', 'index');
		echo "</div>";
		
		$ace_config['currently_viewing_dynamic_overwrite'] = "blog";
		if(isset($fulwidth_replace))
		{
			...
			$ace_config['layout'] = $temp_layout;
			$ace_config['content_class'] = $temp_content;
			$ace_config['sidebar_class'] = $temp_sb;
		}
		echo "</div>";
    	...
    	ob_end_clean();
    	wp_reset_query();
    	unset($ace_config['remove_pagination'], $ace_config['new_query']);
	...
	*/
	function shop($element)
	{
		...
		global $ace_config, $more, $woocommerce_loop;
		
		if($shop_columns == 5 && strpos($ace_config['layout'],'dual') !== false ) $shop_columns = 4;
		
		if($shop_text == 'yes') $ace_config['shop_overview_excerpt'] = 'active';
		$woocommerce_loop['columns'] = $ace_config['shop_overview_column'] = $shop_columns;
	
		$ace_config['new_query'] = array(
			'post_type'	=> 'product',
			'post_status' => 'publish',
		...
		{
			$ace_config['new_query']['post_type'] = "product";
		}
		else
		{		
			$ace_config['new_query']['tax_query'] = array( array( 'taxonomy' => 'product_cat', 
																			 'field' => 'id', 
		...
		query_posts($ace_config['new_query']);
		ob_start();
		woocommerce_get_template_part( 'loop', 'shop' );
		$products = ob_get_clean();
		$extraClass = "dynamic_el_".$this->dynamic_counter;
		
		$output .= "<div class='$extraClass container_wrap ".$ace_config['layout']." template-shop shop_columns_".$ace_config['shop_overview_column']."'>";
		$output .= "<div class='template-shop content shop_slider_$shop_slider' data-interval='$shop_autorotate'>";
		$output .= $products;
		if($shop_pagination == 'yes') $output .= ace_pagination();	
		$output .= "</div>";
		$output .= "</div>";
	...
	*/
	...
	function portfolio($element)
	{
		...
		$ace_config['portfolio_columns'] = $portfolio_columns;			
		$ace_config['portfolio_item_count'] = $portfolio_item_count;	
		...
		{		
			$ace_config['new_query'] = array(	"paged" => get_query_var( 'paged' ), 
												"posts_per_page" => $ace_config['portfolio_item_count'],  
		...
		{
			$ace_config['remove_portfolio_text'] = true;
		}
		
		if($portfolio_sorting != 'yes') $ace_config['portfolio_sorting'] = false;
		...
		ob_start(); //start buffering the output instead of echoing it
		echo "<div class='$extraClass template-portfolio-overview content portfolio-size-".$ace_config['portfolio_columns']." '>";
    	...
    	ob_end_clean();
    	wp_reset_query();
    	unset($ace_config['remove_pagination'], $ace_config['remove_portfolio_text'], $ace_config['new_query']);
	...
	function post_page($element)
	{
		...
		if($additional_loop->have_posts())
		{
			...
			{ 
				...
				{
					$content = get_the_content('<span class="inner_more">'.__('Read more  &rarr;','ace_framework').'</span>');
				...
				else
				{
					...
					$content .= '<p><a class="more-link" href="'. get_permalink().'"><span class="inner_more">'.__('Read more  &rarr;','ace_framework').'</span></a></p>';
				}
	...
	function column_helper_loop_over_posts($data)
	{
		...
		if($additional_loop->have_posts())
		{
			...
			{ 
				...
				{
					$output .= "<div class='entry-content'>";
					$output .= "<h3 class='dynamic-column-title'><a href='".$link."' rel='bookmark' title='".__('Permanent link:','ace_framework')." ".get_the_title()."'>".get_the_title()."</a></h3>";
					$output.= '</div>';
				}
...
*
*/
function ace_get_template()
{
	global $ace_config, $post;
	$dynamic_id = "";
	...
	*/
	$frontpage_switch = ace_get_option('frontpage');
	if($frontpage_switch && isset($ace_config['new_query']) && $ace_config['new_query']['page_id'] == $frontpage_switch)
	{
	 ...
	 */
	if(ace_special_dynamic_template($dynamic_id) && ( is_singular() || isset($ace_config['new_query'])))
	{
	...
	//wpml prepared
	$blog_page_id = ace_get_option('blogpage');
    if (function_exists('icl_object_id'))
    {
        $blog_page_id = icl_object_id($blog_page_id, 'page', true);
    }

	if(ace_get_option('frontpage') != "" && $blog_page_id == $post->ID && !isset($ace_config['new_query']))
	{ 	
	...
	*  in the theme portfolio options 
	...
	if($portfolios = ace_get_option('portfolio'))
	{
		if(!empty($portfolios[0]['portfolio_page']))
		{
			...
			{	
				...
				{	
					$ace_config['portfolio_columns'] = $portfolio['portfolio_columns'];			
					$ace_config['portfolio_item_count'] = $portfolio['portfolio_item_count'];	
					$ace_config['portfolio_sorting'] = $portfolio['portfolio_sorting'];	
					$ace_config['portfolio_layout'] = $portfolio['portfolio_layout'];
					...
					//$ace_config['portfolio_style'] = $portfolio['portfolio_style'];
					...
					if(isset($portfolio['portfolio_cats']))
					{		
					$ace_config['new_query'] = array("paged" => get_query_var( 'paged' ), "posts_per_page" => $portfolio['portfolio_item_count'],  'tax_query' => array( array( 'taxonomy' => 'portfolio_entries', 'field' => 'id', 'terms' => $terms, 'operator' => 'IN')));
					}
...
function ace_get_frontpage_template()
{
	...
	{ 
		...
		$ace_config['new_query'] = array("page_id"=> ace_get_option('frontpage'), "paged" => $paged);
				
		$custom_fields = get_post_meta(ace_get_option('frontpage'), '_wp_page_template', true);
...
/*
* support function that checks if the current page should have a post or page layout and returns the string so ace_template_set_page_layout can check it
*/
function ace_template_helper_get_layout_string($post_type = "")
{
	
	//$post_type should either be 'page_layout' or 'blog_layout'
	if(!$post_type) $post_type = 'blog_layout';
	if(is_page() && !ace_is_overview()) $post_type = 'page_layout';
	if(is_front_page() && ace_get_option('frontpage') != "") $post_type = 'page_layout';
...
/*
* support function that checks if the current page should have a post or page layout and sets the var $ace_config['layout']
*/
function ace_template_set_page_layout($post_type = '', $layout_string = false)
{
	global $ace_config;
	...
	{
		if(!$post_type) $post_type = ace_template_helper_get_layout_string();
	
		//get the global page layout option set in your backend
	 	 $result = ace_get_option($post_type, 'sidebar_class|three : content_class|nine alpha : layout|sidebar_right');
	
	 	 //overwrite the global setting with the page single setting, in case one is defined
	 	 $post_id = ace_get_the_ID();
	
	 	 if($post_id && $new = ace_post_meta($post_id, 'layout')) $result = $new;
 	 }
 	 ...
 	 foreach($results as $result)
 	 {
 	 	...
 	 	$ace_config[$result[0]] = $result[1];
 	 }
 	 
 	 $ace_config['layout'] = apply_filters('ace_layout_filter',  $ace_config['layout']);
 	 
}


/*
* sicnce the theme needs not only to check if a dynamic template was selected but also if the page layout is set to dynamic here is a improved version of ace_is_dynamic_template
*/
if(!function_exists('ace_special_dynamic_template')){

	function ace_special_dynamic_template($id = false)
	{
		$return = false;
		...
		if(ace_is_dynamic_template($id) && (ace_post_meta($id, 'layout') == 'layout|dynamic' ) ) $return = true;
