...
class ace_slideshow
{
	...
	var $type;				// slidehsow type: eg aceslider, fade slider, etc
	...
	*/
	...
	function ace_slideshow($post_id = false, $showcaption = true)
	{
		global $ace_config;
		
		///if no id was passed get it 
		if(!$post_id) $post_id = ace_get_the_ID();
		if(!$post_id) return false;
		
		$this->defaultSlider 	= 'fade_slider';
		$this->post_id 			= $post_id;
		$this->slides 			= ace_post_meta($this->post_id, 'slideshow'); 
		$this->type 			= ace_post_meta($this->post_id, '_slideshow_type');
		$this->position 		= ace_post_meta($this->post_id, '_slideshow_position');
		$this->autoplay 		= ace_post_meta($this->post_id, '_slideshow_autoplay');
		$this->duration 		= ace_post_meta($this->post_id, '_slideshow_duration');
		$this->slideshow_poster = ace_get_option('slideshow_poster');
	...
	function modify_slide_poster($set_new = false)
	{
		...
		if($this->slideshow_poster == 'single' && (ace_is_overview() || $set_new))
		{
	...
	{
		if(($this->position == 'big' && !ace_is_overview()) && !$force) return false;
		if(ace_is_overview()) $this->autoplay = "false";
	...
	function create_button($slide, $key)
	{
		...
		{
			$button = ace_get_link($slide, $key, false, $this->post_id);
		}
		
		if($button) $button = "<a class='ace-button on-primary-color primary-background primary-border caption-slideshow-button $nextSlide ".$key."class' href='$button' title='' >".$slide[$key.'title']."</a>";
		...
		{ 
			...
			{	
				...
				if(!empty($slide['slideshow_image']))
				{
					...
					$image_string = ace_image_by_id($slide['slideshow_image'], $this->img_size); 
					...
					if(empty($this->link_overwrite))
					{
						...
						$image = ace_get_link($slide, 'slideshow_', $image_string, $this->post_id);
					}
					else
					{
				...
				{
					if(ace_backend_is_file($slide['slideshow_video'], 'html5video'))
					{
						$video = ace_html5_video_embed($slide['slideshow_video']);
					}
		...
		{ 
			...
			{	
				...
				{	
					...
					//get the image by passing the attachment id.
					$image = ace_image_by_id($slide['slideshow_image'],$thumbsize);
...
######################################################################
	...
	function activate_piecemaker()
	{
		...
		$output  = "";
		if(empty($ace_config['swf_embed_loaded'])){
			$output .= '<script type="text/javascript" src="'.ACE_BASE_URL.'slideshow/scripts/swfobject/swfobject.js"></script>'."\n";
			$ace_config['swf_embed_loaded'] = true;
		}
		...
		$output .= '
					<script type="text/javascript">
				      ...
				      var flashvars = {};
				      flashvars.cssSource = "'.ACE_BASE_URL.'slideshow/piecemaker.css";
				      flashvars.xmlSource = "'.ACE_BASE_URL.'slideshow/piecemaker.xml.php?post_id='.$this->post_id.'";
				      ...
				      swfobject.embedSWF("'.ACE_BASE_URL.'slideshow/piecemaker.swf", "slideshow_'.$this->type.$this->post_id.'", "'.($ace_config['imgSize']['featured']['width']+100).'", "'.($ace_config['imgSize']['featured']['height']+70).'", "10", null, flashvars,    
				      params, null);
	...
	function _build_xml_slides($element)
	{
		...
		//apply links to the image if thats what the user wanted
		$url = ace_get_link($element, 'slideshow_', false, $this->post_id);
		if($url) $url = "<Hyperlink URL='".$url."'  />";
	
		if(is_numeric($element['slideshow_image']))
		{
		
			$this->slides_xml .= '<Image Source="'.ace_image_by_id($element['slideshow_image'], 'featured', 'url').'" Title="'.$element['slideshow_caption_title'].'">';
			$this->slides_xml .= $text.$url;
			$this->slides_xml .= '</Image>';
		}
		else
		{
			if(ace_backend_is_file($element['slideshow_image'], 'html5video') || ace_backend_is_file($element['slideshow_image'], array('swf')))
			{
				...
				$this->slides_xml .= '<'.$tag.' Source="'.$element['slideshow_image'].'" Title="'.$element['slideshow_caption_title'].'" Width="'.$ace_config['imgSize']['featured']['width'].'" Height="'.$ace_config['imgSize']['featured']['height'].'" Autoplay="true" >';
				
				$image = $path_split[1].'.jpg';
				$checkpath = $path_split[1].'-'.$ace_config['imgSize']['featured']['width'].'x'.$ace_config['imgSize']['featured']['height'].'.jpg';
	...
	{
		global $ace_config;
		//$color_1 = substr(ace_get_option('color_1','#000000'),1);
		//$color_3 = substr(ace_get_option('color_3','#ffffff'),1);
		$color_1 = "000000";
		$color_3 = "ffffff";
